		...
		try {
			Patient.findOne({id: req.params.id}).then((patient)=>{                
				if(patient !== null){
					...
					return apiResponse.successResponseWithData(res, "Operation success", patientData);
				}else{
					return apiResponse.successResponseWithData(res, "Operation success", {});
				}
			});
		...
		try {
			...
			if (!errors.isEmpty()) {
				...
				patient.save(function (err) {
					...
					let patientData = new PatientData(patient);
					return apiResponse.successResponseWithData(res,"Patient add Success.", patientData);
				});
			}
		} catch (err) {
			//throw error in json response with status 500. 
...
exports.patientUpdate = [
	...
	body("isbn", "ISBN must not be empty").isLength({ min: 1 }).trim().custom((value,{req}) => {
		return Patient.findOne({isbn : value,user: req.user._id, _id: { "$ne": req.params.id }}).then(patient => {
			if (patient) {
							...
							if(foundPatient.user.toString() !== req.user._id){
								...
								Patient.findByIdAndUpdate(req.params.id, patient, {},function (err) {
									if (err) { 
										...
										let patientData = new PatientData(patient);
										return apiResponse.successResponseWithData(res,"Patient update Success.", patientData);
									}
								});
							}
						}
